...
<div class="container">
    ...
    if (isset($_SESSION['user_id'])) {
        // Load JSON file
        $json = file_get_contents('web_dev.json');
    ...
    if (isset($_POST['proficiencyLevel'])) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO user_proficiency (id, web_dev) VALUES (:user_id, :web_dev) ON DUPLICATE KEY UPDATE web_dev = VALUES(web_dev)");

            // Bind parameters and execute statement
            $stmt->bindParam(':user_id', $user_id);
            $stmt->bindParam(':web_dev', $proficiencyLevel);
            $stmt->execute();
...
<script>
    ...
    function submitQuiz() {
        ...
        xhr.open("POST", "web_devEvaluation.php", true);
