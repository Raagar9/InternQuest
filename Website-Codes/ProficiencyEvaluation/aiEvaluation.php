...
<div class="container">
    ...
    if (isset($_POST['proficiencyLevel'])) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO user_proficiency (id, ai) VALUES (:user_id, :ai) ON DUPLICATE KEY UPDATE ai = VALUES(ai)");
            $stmt2 = $pdo->prepare("INSERT INTO scores (id, ai) VALUES (:user_id, :aiScore) ON DUPLICATE KEY UPDATE ai = VALUES(ai)");

            $stmt->bindParam(':user_id', $user_id);
            $stmt->bindParam(':ai', $proficiencyLevel);
            $stmt->execute();

            $stmt2->bindParam(':user_id', $user_id);
            $stmt2->bindParam(':aiScore', $scorePercentage);
            $stmt2->execute();
